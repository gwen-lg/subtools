                        ...
                        match encoding.encoding() {
                            ...
                            ContentEncodingValue::Compression(comp) => {
                                if comp.algo() == ContentCompAlgo::Zlib {
                                    assert!(comp.settings().is_none(), "Invalid decompress algo");
                                    Box::new(|data: Vec<u8>| decompress_zlib(data.as_slice()))
...
fn decompress_zlib(input: &[u8]) -> Vec<u8> {
    ...
    let mut zwriter = flate2::bufread::ZlibDecoder::new(input);
